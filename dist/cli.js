...
import { CLI_NAME, DEFAULT_FC_QUERY_URL, GITHUB_REPO, PRODUCT, RETRY_HINT, SITE, VERSION, } from "./constants.js";
...
function parseGlobalFlags(argv) {
    ...
    const rest = argv.filter((a) => !["--force", "--yes", "-y", "--dry-run", "--strict", "--version", "--help", "-h"].includes(a));
...
async function main() {
    const { force, yes, dryRun, strict, version, help, rest } = parseGlobalFlags(process.argv.slice(2));
    if (version) {
