...
test("extractAddTarget", () => {
    assert.equal(extractAddTarget(["--profile", "web", "add", "github:a/b"]), "github:a/b");
...
test("extractAddTarget skips value-taking flags", () => {
    ...
    assert.equal(extractAddTarget(["--profile", "web", "add", "--tag", "v1.0", "github:a/b"]), "github:a/b");
    // inline --key=value form
    assert.equal(extractAddTarget(["add", "--tag=v1.0", "github:a/b"]), "github:a/b");
    // --branch with value
    assert.equal(extractAddTarget(["add", "--branch", "dev", "github:a/b"]), "github:a/b");
});
